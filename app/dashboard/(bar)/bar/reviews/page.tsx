...
const ReviewProfileModal = ({
  ...
  const loadProfile = async (cursor: ReviewCursor = null) => {
    ...
    setLoading(true);
    try {
      const res = await barService.getReviewProfile(eventId, {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="relative flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-2xl">
        <button onClick={onClose} className="absolute right-6 top-6 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-white/90 text-gray-700 shadow-sm hover:bg-white">
            <X size={15} />
        </button>
            ...
            <div className="px-6 py-5">
              ...
              <section className="mb-7">
                ...
                <div className="space-y-3 text-xs text-black">
                  <div className="flex items-center gap-2"><StarIcon /> Performance Quality: {formatRating(ratings.performanceQuality)}</div>
                  <div className="flex items-center gap-2"><StarIcon /> Crowd Engagement: {formatRating(ratings.crowdEngagement)}</div>
                  <div className="flex items-center gap-2"><StarIcon /> Professionalism: {formatRating(ratings.professionalism)}</div>
                </div>
              </section>
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#fff', padding: '40px 48px' }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-start', gap: '16px', marginBottom: '48px' }}>
          <EventStatCard label="Total Reviews" value={totalReviews} />
          <EventStatCard label="Public Events" value={publicEventCount} />
          <EventStatCard label="Private Events" value={privateEventCount} />
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {['ID', 'ENTERTAINER', 'TYPE', 'EVENT NAME', 'EVENT TYPE', 'NO. OF REVIEWS', 'AVG. RATING', 'ACTION'].map((header) => (
                  <th key={header} style={{
                    padding: '20px 16px', textAlign: header === 'ACTION' ? 'right' : 'left',
            ...
            <tbody>
              {filteredRows.length ? filteredRows.map((row, index) => (
                ...
                >
                  <td style={{ padding: '24px 16px', fontSize: '13px', color: '#111827' }}>{String(index + 1).padStart(2, '0')}</td>
                  <td style={{ padding: '24px 16px', fontSize: '13px', fontWeight: 500, color: '#111827' }}>{row.entertainer || '—'}</td>
                  <td style={{ padding: '24px 16px' }}>
                    <span style={{
                      display: 'inline-flex', padding: '6px 16px', borderRadius: '99px',
                      fontSize: '12px', fontWeight: 500, backgroundColor: '#e5e7eb',
                  ...
                  <td style={{ padding: '24px 16px', fontSize: '13px', color: '#4b5563' }}>{row.eventName || '—'}</td>
                  <td style={{ padding: '24px 16px' }}>
                    <span style={{
                      display: 'inline-flex', padding: '6px 16px', borderRadius: '99px',
                  ...
                  <td style={{ padding: '24px 16px', fontSize: '13px', color: '#4b5563' }}>{row.noOfReviews ?? 0}</td>
                  <td style={{ padding: '24px 16px', fontSize: '13px', fontWeight: 500, color: '#111827' }}>{formatRating(row.avgRating)}</td>
